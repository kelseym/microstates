...
% filename = {'C:\Projects\McDonnell\MCD12_WAKEA.EDF',...
...
%               'SWS',...
%               'N2'};
% sensorList = [1:37,39:48,50:56,58:64];
% % sensorList = [1:64];
...
if showNSecPlots > 0
    ...
    for i=1:size(microstateTemplates,1)
        ...
        figure;
        ft_plot_lay(lay, 'point', 'no', 'box', 'no', 'label', 'no', 'mask', 'no');
        xPos = lay.pos(1:32,1);
        yPos = lay.pos(1:32,2);
...
for gi=get(gca,'XTick')
   ...
   for bi=1:length(gWidth)
    ...
    x = x+(rand(size(x))-0.5)*0.04; % Add jitter to x for visibility
