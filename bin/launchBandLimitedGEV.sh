...
do
  ...
  echo $EXPERIMENTID
  qsub -V -q dque -l walltime=23:59:59,mem=12gb,vmem=12gb  -o $LOGDIR/OUTLOG_$PIPELINE"_"$EXPERIMENTID.log -e $LOGDIR/ERRORLOG_$PIPELINE"_"$EXPERIMENTID.log $QSUBDIR/$PIPELINE.pbs > $LOGDIR/jobids-$PIPELINE"_"$EXPERIMENTID.txt
done
